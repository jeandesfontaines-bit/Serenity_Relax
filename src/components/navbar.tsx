      ...
      <nav className="max-w-6xl mx-auto bg-white/80 backdrop-blur-3xl border border-white/40 shadow-2xl shadow-onyx/5 rounded-full px-8 py-3 transform transition-all hover:scale-[1.01]">
        <div className="flex justify-between items-center">
          <Link href="/" className="flex flex-col items-start leading-none group">
            ...
            <span className="font-heading text-[9px] font-semibold uppercase tracking-[0.4em] text-onyx/40">Therapy Genève</span>
          </Link>
          ...
          <div className="hidden md:flex items-center gap-10">
            {isTherapistArea ? (
              adminLinks.map((link) => (
                <Link
                  key={link.id}
                  href={link.href}
                  className={`font-heading text-[10px] font-semibold uppercase tracking-widest transition-colors ${pathname === link.href ? 'text-fresh-green' : 'text-onyx/60 hover:text-onyx'}`}
                >
                  {link.label}
                </Link>
              ))
              ...
              <div className="flex items-center gap-10">
                <Link href="#services" className="font-heading text-[10px] font-semibold uppercase tracking-widest text-onyx/40 hover:text-onyx transition-all">Les Soins</Link>
                {isTherapist && <Link href="/therapist/dashboard" className="font-heading text-[10px] font-semibold uppercase tracking-widest text-fresh-green">Admin</Link>}

                <button
                  onClick={onBookingClick}
                  className="h-10 px-8 bg-ochre text-white font-heading text-[10px] font-semibold uppercase tracking-widest rounded-full transition-all hover:shadow-xl hover:shadow-ochre/20"
                >
                  Réserver
                </button>
              </div>
            ...
            {effectiveUser ? (
              <div className="flex items-center gap-4">
                <span className="font-heading text-[10px] font-semibold uppercase tracking-widest text-onyx">{effectiveUser.name}</span>
          ...
          {isMenuOpen && (
            ...
            >
              ...
              <button
                ...
                className="h-14 bg-onyx text-sandstone font-heading text-[11px] font-semibold uppercase tracking-widest rounded-full"
              >
                Prendre rendez-vous
              </button>
              {effectiveUser ? (
                <button onClick={handleSignOut} className="font-heading text-[9px] font-semibold uppercase tracking-[0.3em] text-onyx/30">Déconnexion</button>
              ) : (
                <Link href="/login" onClick={() => setIsMenuOpen(false)} className="font-heading text-[9px] font-semibold uppercase tracking-[0.3em] text-onyx/30">Accès Client</Link>
              )}
            </motion.div>
          )}
        </AnimatePresence>
