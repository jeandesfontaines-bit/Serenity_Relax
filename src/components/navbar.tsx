...
export function Navbar({ onBookingClick }: NavbarProps) {
  ...
  return (
    <div className="absolute top-6 left-0 right-0 z-[150] px-6">
      <nav className="max-w-4xl mx-auto bg-white/70 backdrop-blur-xl border border-white/20 shadow-[0_8px_32px_rgba(0,0,0,0.05)] rounded-full px-8 py-1.5 md:py-2">
        <div className="flex justify-between items-center">
          <Link href="/" className="flex items-baseline gap-1 cursor-pointer group">
            <span className="font-sans font-bold text-[0.7rem] tracking-[0.3em] text-neutral-900 uppercase md:text-[0.75rem] lg:text-[0.8rem]">SERENITY RELAX</span>
            <span className="font-cursive text-[1.1rem] text-neutral-900 ml-1 md:text-[1.25rem] lg:text-[1.35rem]">by João</span>
          </Link>
          ...
          <div className="hidden md:flex items-center gap-8 lg:gap-12">
            {isTherapistArea ? (
              <>
                {adminLinks.map((link) => (
                  <Link 
                    key={link.id} 
                    href={link.href} 
                    className={`text-[0.65rem] font-sans font-black uppercase tracking-[0.18em] transition-colors md:text-[0.7rem] lg:text-[0.75rem] ${pathname === link.href ? 'text-neutral-900' : 'text-neutral-400 hover:text-neutral-900'}`}
                  >
                    {link.label}
                  </Link>
                ))}
              ...
              <>
                <Link 
                  href="/client/portal" 
                  className={`text-[0.65rem] font-sans font-black uppercase tracking-[0.18em] transition-all md:text-[0.7rem] lg:text-[0.75rem] ${pathname === '/client/portal' ? 'text-neutral-900' : 'text-neutral-400 hover:text-neutral-900'}`}
                >
                  Espace Privé
                </Link>
                <button 
                  onClick={onBookingClick}
                  className="flex items-center justify-center text-[0.65rem] font-sans font-black uppercase tracking-[0.18em] transition-all px-4 py-1.5 rounded-full border border-neutral-900 md:text-[0.7rem] lg:text-[0.75rem] text-neutral-900 hover:bg-neutral-900 hover:text-white"
                >
                  Réserver
                </button>
              </>
