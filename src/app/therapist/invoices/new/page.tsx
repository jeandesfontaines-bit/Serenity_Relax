    ...
    <>
      ...
      <div className="min-h-screen bg-[#F8F5F0] pt-32 pb-24 px-6 md:px-12">
        <div className="max-w-4xl mx-auto space-y-10">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="dash-card p-12 border border-white relative overflow-hidden">
            ...
            <form onSubmit={handleSubmit} className="space-y-12">
               ...
               <div className="space-y-6">
                  ...
                  <div className="relative group">
                     <User className="absolute left-8 top-1/2 -translate-y-1/2 text-gray-300 group-hover:text-[#5F27CD] transition-colors" size={20} />
                     <input 
                       required
                       type="text" 
               ...
               <div className="space-y-6">
                  ...
                  <div className="space-y-4">
                     {invoice.items.map((item, idx) => (
                       <div key={idx} className="flex gap-4">
                          <div className="flex-1 relative group">
                             <Sparkles className="absolute left-6 top-1/2 -translate-y-1/2 text-gray-300 group-hover:text-[#0ABDE3] transition-colors" size={16} />
                             <input 
                               type="text" 
                               placeholder="Description du soin..." 
                               value={item.description}
                               onChange={e => updateItem(idx, { description: e.target.value })}
                               className="w-full h-16 bg-white border border-gray-100 rounded-2xl pl-16 pr-6 focus:outline-none focus:border-[#0ABDE3] transition-all"
                             />
                          </div>
                          <div className="w-32">
                             <input 
                               ...
                               className="w-full h-16 bg-white border border-gray-100 rounded-2xl px-6 text-center font-bold focus:outline-none focus:border-[#0ABDE3] transition-all"
                             />
                          </div>
                     ...
                     <button type="button" onClick={addItem} className="flex items-center gap-3 px-8 py-4 bg-white border border-dashed border-gray-200 rounded-2xl text-[0.65rem] font-black uppercase tracking-widest text-[#5F27CD] hover:bg-indigo-50/30 transition-all">
               ...
               <div className="pt-12 border-t border-gray-100 flex flex-col md:flex-row justify-between items-center gap-10">
                  <div className="text-center md:text-left">
                     ...
                     <p className="text-6xl font-light text-[#222F3E]">{total} <small className="text-xl font-black opacity-20">CHF</small></p>
                  </div>
                  
                  <div className="flex gap-4">
                     {success ? (
                       <div className="flex items-center gap-4 py-8 px-12 bg-emerald-50 text-emerald-600 rounded-[2.5rem] font-black uppercase text-[0.7rem] tracking-widest animate-bounce">
                       ...
                       <button 
                         disabled={loading}
                         type="submit"
                         className="h-24 px-16 bg-[#222F3E] text-white rounded-[2.5rem] shadow-2xl shadow-indigo-200/50 flex items-center justify-center gap-4 group hover:scale-105 active:scale-95 transition-all text-[0.8rem] font-black uppercase tracking-[0.3em]"
                       >
                         {loading ? "Traitement..." : <><Save size={24} className="text-[#1DD1A1] group-hover:rotate-12 transition-transform" /> Émettre la Facture</>}
                       </button>
                     )}
                  </div>
               </div>
