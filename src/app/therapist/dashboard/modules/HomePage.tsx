...
export default function HomePage({ appointments, monthlyGoal, onSelectAppt, onNavigate, onEditGoal }: HomePageProps) {
  ...
  const getApptDate = (a: Appointment) => a.date || (a.startTime ? a.startTime.split('T')[0] : '');
  const getApptTime = (a: Appointment) => a.time || (a.startTime ? a.startTime.split('T')[1].substring(0, 5) : '');

  const todayAppts = appointments
    .filter(a => getApptDate(a) === today)
    .sort((a, b) => getApptTime(a).localeCompare(getApptTime(b)));

  const totalToday = todayAppts.length;
  const revenueToday = todayAppts.reduce((sum, a) => sum + (a.price || a.priceSnapshot || 0), 0);
  
  // Calculate current month's total revenue for goal tracking
  const currentMonth = new Date().toISOString().slice(0, 7); // "YYYY-MM"
  const monthRevenue = appointments
    .filter(a => getApptDate(a).startsWith(currentMonth))
    .reduce((sum, a) => sum + (a.price || a.priceSnapshot || 0), 0);
      ...
      <div className="space-y-8 pt-8 px-2">
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-6">
          <div className="space-y-2">
            <h2 className="text-3xl font-sans font-medium text-[#222F3E] tracking-tight">Agenda du <span className="italic">jour</span></h2>
              ...
              >
                ...
                <div className="flex items-center justify-between sm:justify-end gap-6 sm:pl-6 sm:border-l border-gray-100">
                  <div className="flex flex-col items-end">
                    ...
                    <p className="text-base font-sans font-medium mt-1 text-gray-500">{appt.price || 150} CHF</p>
                  </div>
                </div>
              </motion.div>
            ))
