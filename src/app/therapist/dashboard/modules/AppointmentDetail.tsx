...
export default function AppointmentDetail({
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-0 sm:p-4 overflow-hidden">
      ...
      >
            {/* HEADER IMPACT LUXE */}
            <div className="bg-gradient-to-r from-[#5F27CD] via-[#0ABDE3] to-[#1DD1A1] px-6 py-6 text-white relative">
              <button
                onClick={onClose}
            ...
            <div className="p-8 lg:p-12 space-y-10">
              ...
              {/* Payment Hub */}
              <div className="bg-white/60 p-8 rounded-[2.5rem] border border-white shadow-xl shadow-indigo-100/20">
                    ...
                    <AnimatePresence mode="wait">
                      {showPaymentSelector ? (
                        ...
                        >
                          {(['Twint', 'Card', 'Cash'] as const).map(m => (
                            <button
                              ...
                              className="flex-1 min-w-[100px] py-5 rounded-2xl border border-gray-100 hover:border-[#1DD1A1] hover:bg-[#1DD1A1] hover:text-white transition-all text-sm font-bold shadow-sm"
                            >
                              {m}
                            </button>
                          ))}
                        ...
                        >
                          ...
                          <ChevronRight size={20} className="text-gray-200 group-hover:text-[#5F27CD] transition-all" />
                        </motion.div>
                      )}
                    </AnimatePresence>
                  </div>
              ...
              {current.clientId && onGoToClient && (
                <button
                  ...
                  className="w-full flex items-center justify-between p-8 bg-gradient-to-r from-gray-50 to-white border border-white rounded-[2rem] hover:shadow-2xl hover:-translate-y-1 transition-all group"
                >
                  <div className="flex items-center gap-4 text-left">
                    <div className="w-12 h-12 rounded-xl bg-[#222F3E] text-white flex items-center justify-center group-hover:bg-[#5F27CD] transition-all">
                      <ShieldCheck size={24} />
                    </div>
                    <div>
          ...
          <div className="pt-10">
            <button 
              ...
              className="w-full flex items-center justify-center gap-4 py-8 bg-[#222F3E] text-white rounded-[2.5rem] shadow-2xl shadow-indigo-200/50 group hover:scale-[1.02] active:scale-95 transition-all text-[0.7rem] font-black uppercase tracking-[0.3em]"
            >
              <FileText size={20} className="text-[#0ABDE3] group-hover:rotate-6 transition-transform" />
