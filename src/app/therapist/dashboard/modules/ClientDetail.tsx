...
const STATUS_META: Record<ClientStatus, { label: string; className: string }> = {
  ...
  hiatus: { label: 'En pause', className: 'border border-[#cbd5e1] bg-[#f8fafc] text-[#475569]' },
  active: { label: 'Client actif', className: 'border border-[#c7d2fe] bg-[#eef2ff] text-[#4338ca]' },
};
...
export default function ClientDetail({
  ...
  return (
    <div className="flex-1 overflow-auto bg-transparent text-[#0f172a]">
        ...
        <div className="grid grid-cols-1 gap-6 xl:grid-cols-[340px_minmax(0,1fr)]">
          <aside className="space-y-6 xl:sticky xl:top-6 self-start">
            <section className={`overflow-hidden ${dashboardPanel}`}>
              <div className="border-b border-[#e2e8f0] bg-[linear-gradient(180deg,rgba(248,250,252,0.98),rgba(255,255,255,0.96))] p-6">
                <div className="flex h-28 w-28 items-center justify-center rounded-[32px] bg-white text-[42px] font-semibold text-[#0f172a] shadow-[0_16px_32px_rgba(15,23,42,0.06)]">
                  {clientInitials}
                </div>
                <h2 className="mt-6 text-[2.05rem] font-semibold leading-[0.95] tracking-tight text-[#0f172a]">
                  {fullName}
                </h2>
                <p className="mt-3 text-base font-medium text-[#4f46e5]">
                ...
                <div className="mt-5 flex flex-wrap gap-2">
                  ...
                  <span className="rounded-full border border-[#e2e8f0] bg-white/92 px-3 py-1.5 text-[10px] font-semibold uppercase tracking-[0.18em] text-[#475569]">
                    {preferredService}
                  </span>
                </div>
              </div>
              ...
              <div className="space-y-5 p-6">
                ...
                <div className="grid gap-3 pt-3">
                  <button
                    ...
                    className="flex items-center justify-center gap-2 rounded-[18px] border border-[#dbe3ef] bg-white px-5 py-4 text-sm font-semibold text-[#1f2937] transition-all hover:border-[#c7d2fe] hover:bg-[#f8faff] hover:text-[#312e81]"
                  >
                  ...
                  <button
                    onClick={onClose}
                    className="rounded-[16px] border border-[#dbe3ef] bg-white/88 px-5 py-3 text-[12px] font-semibold uppercase tracking-[0.18em] text-[#475569] transition-all duration-200 hover:border-[#c7d2fe] hover:bg-[#f8faff] hover:text-[#312e81]"
                  >
                    Retour
                  </button>
                </div>
            ...
            <section className={`${dashboardPanel} p-5`}>
              ...
              <div className="grid grid-cols-1 gap-2 rounded-[22px] border border-[#e2e8f0] bg-white/80 p-3 sm:grid-cols-2 xl:grid-cols-1">
          ...
          <div className="space-y-6">
            <section className={`${dashboardPanel} p-6`}>
              <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
                <div>
                  ...
                  <h3 className="mt-2 text-[2.2rem] font-semibold italic leading-none tracking-tight text-[#0f172a]">
                    Prochains soins
                  </h3>
                </div>
                {urgencyCount > 0 ? (
                  <span className="rounded-full border border-[#c7d2fe] bg-[#eef2ff] px-4 py-2 text-sm font-semibold text-[#4338ca]">
            ...
            <div className={dashboardTableSectionHeader}>
              ...
              <button
                ...
                className="text-xs font-bold uppercase tracking-[0.18em] text-[#4f46e5] hover:underline"
              >
                Journal complet
              </button>
            </div>
            ...
            {completedAppts.length > 0 ? (
              <div className="overflow-x-auto">
                <table className="min-w-full text-left">
                  <thead className={`border-b border-[#e2e8f0] ${dashboardTableHeader}`}>
                    <tr>
                  ...
                  <tbody className="divide-y divide-[#e2e8f0] text-sm">
                    {completedAppts.map((appt) => (
                      ...
                      >
                        <td className={`${dashboardTableCell} font-medium text-[#0f172a]`}>
                          <div className="flex items-center gap-2">
                            <span className="text-[#6366f1]">{materialIcon('spa')}</span>
                            <span>{appt.serviceName || 'Seance'}</span>
                          </div>
                        </td>
                        <td className={`${dashboardTableCell} text-[#475569]`}>{appt.date ? formatDisplayDate(appt.date) : '—'}</td>
                        <td className={`${dashboardTableCell} text-[#475569]`}>{appt.time || '—'}</td>
                        <td className={`${dashboardTableCell} text-[#475569]`}>{appt.duration || '60 min'}</td>
                        ...
                        <td className={`${dashboardTableCell} text-right text-[#94a3b8]`}>
            ...
            {incompleteAppts.length > 0 && (
              <div className="border-t border-[#e2e8f0] bg-[#fbfcff] px-6 py-5">
                <p className="mb-3 text-[11px] font-bold uppercase tracking-[0.2em] text-[#94a3b8]">
                  Seances incompletes
                </p>
                <div className="space-y-2">
                  {incompleteAppts.map((appt) => (
                    <button
                      key={appt.id}
                      onClick={() => onSelectAppt(appt)}
                      className="flex w-full items-center justify-between rounded-[18px] border border-[#e2e8f0] bg-white px-4 py-3 text-left transition-colors hover:bg-[#f8faff]"
                    >
                      <div>
                        <p className="text-sm font-medium text-[#0f172a]">{appt.serviceName || 'Seance incomplete'}</p>
                        <p className="mt-1 text-xs text-[#64748b]">Date ou heure manquante</p>
                      </div>
                      <ChevronRight size={16} strokeWidth={1.5} className="text-[#94a3b8]" />
                    </button>
                  ))}
                </div>
              </div>
            ...
            <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_320px]">
              <section className={`flex flex-col p-6 ${dashboardPanel}`}>
                <div className="mb-4 flex items-center justify-between border-b border-[#e2e8f0] pb-4">
                  ...
                  <span className="inline-flex items-center gap-1.5 rounded-full border border-[#c7d2fe] bg-[#eef2ff] px-3 py-1 text-[11px] font-bold uppercase tracking-[0.18em] text-[#4338ca]">
                    <ShieldCheck size={12} strokeWidth={1.7} />
                    Confidentiel
                  </span>
                </div>

                <div className="mb-5 rounded-[24px] border border-[#dbe3ef] bg-[linear-gradient(135deg,rgba(255,255,255,0.98),rgba(248,250,252,0.96))] p-5 shadow-[inset_0_1px_0_rgba(255,255,255,0.8)]">
                  ...
                  <p className="border-l-2 border-[#84cc16] pl-4 text-sm leading-6 text-[#475569]">
                ...
                <textarea
                  ...
                  className="min-h-[320px] flex-1 resize-none rounded-[24px] border border-[#dbe3ef] bg-[#f8fafc] p-5 text-sm leading-7 text-[#0f172a] outline-none transition focus:border-[#6366f1] focus:ring-4 focus:ring-[#6366f1]/12"
                />
                ...
                <div className="mt-4 flex items-center justify-between border-t border-[#e2e8f0] pt-4">
                  ...
                  <button
                    ...
                    className="rounded-full bg-gradient-to-r from-[#5b21b6] to-[#6366f1] px-5 py-2 text-xs font-bold uppercase tracking-[0.18em] text-white shadow-[0_10px_24px_rgba(99,102,241,0.2)] transition-opacity hover:opacity-95"
                  >
                    Continuer l'edition
                  </button>
                </div>
                    ...
                    <div className="space-y-3">
                      {unpaidAppts.map((appt) => (
                        <div key={appt.id} className="rounded-[22px] border border-[#fecaca] bg-[linear-gradient(135deg,#fff1f2,#ffffff)] px-4 py-4">
                          <p className="text-sm font-medium text-[#0f172a]">{appt.serviceName || 'Seance'}</p>
                  ...
                  {paidAppts.length > 0 ? (
                    <div className="space-y-3">
                      {paidAppts.slice(0, 8).map((appt) => (
                        <div key={appt.id} className="rounded-[22px] border border-[#e2e8f0] bg-[#fbfcff] px-4 py-4">
                          <div className="flex items-start justify-between gap-3">
                            ...
                            <span className="text-sm font-semibold text-[#0f172a]">{formatMoney(appt.price || 0)}</span>
                          </div>
                          {appt.paymentMethod && (
                            <p className="mt-3 text-[11px] font-bold uppercase tracking-[0.16em] text-[#4f46e5]">
                              {appt.paymentMethod}
                            </p>
                          )}
                        </div>
                    ...
                    <div className="py-10 text-center">
                      <CreditCard size={24} strokeWidth={1.4} className="mx-auto mb-3 text-[#cbd5e1]" />
                      <p className="text-sm text-[#64748b]">Aucun paiement enregistre.</p>
                    </div>
                  )}
                </div>
              </div>
...
function SectionHeader({ icon, title, noMargin = false }: { icon: string; title: string; noMargin?: boolean }) {
  return (
    <div className={`flex items-center gap-2 ${noMargin ? '' : 'mb-4 border-b border-[#e2e8f0] pb-3'}`}>
      <span className="text-[18px] text-[#6366f1]">{materialIcon(icon)}</span>
      <h3 className="text-[11px] font-semibold uppercase tracking-[0.18em] text-[#0f172a]">
        {title}
      </h3>
    </div>
  );
    ...
    <div className={`group relative ${className}`}>
      {editing ? (
        <input
          ...
          onKeyDown={handleKey}
          placeholder={label}
          className="w-full rounded-[16px] border border-[#6366f1] bg-white px-4 py-3 text-sm text-[#0f172a] outline-none ring-4 ring-[#6366f1]/10"
        />
      ) : (
        <div
          onDoubleClick={startEdit}
          title={`Double-cliquer pour modifier ${label}`}
          className="flex min-h-[56px] cursor-text items-center rounded-[16px] border border-transparent bg-white/70 px-4 py-3 text-sm text-[#0f172a] transition group-hover:border-[#dbe3ef] group-hover:bg-[#f8fafc]"
        >
          {value ? <span>{value}</span> : <span className="italic text-[#94a3b8]">{label}</span>}
        </div>
      )}
    </div>
  );
...
function InsightCard({
  ...
  const toneClass =
    accent === 'primary'
      ? 'bg-[linear-gradient(135deg,#eef2ff,#ffffff)] border-[#c7d2fe] text-[#4338ca]'
      : accent === 'neutral'
        ? 'bg-[#f8fafc] border-[#e2e8f0] text-[#475569]'
        : 'bg-[#fbfcff] border-[#e2e8f0] text-[#4338ca]';

  return (
    <div className={`rounded-[22px] border p-4 ${toneClass}`}>
      <p className="mb-2 text-[10px] font-bold uppercase tracking-[0.18em] text-[#94a3b8]">{label}</p>
...
function SummaryChip({
  ...
  return (
    <div className={`rounded-[18px] border px-4 py-3 ${tone === 'alert' ? 'border-[#fecaca] bg-[#fff1f2]' : 'border-[#e2e8f0] bg-[#fbfcff]'}`}>
      <p className="mb-1 text-[10px] font-bold uppercase tracking-[0.18em] text-[#94a3b8]">{label}</p>
      <p className={`text-sm font-medium ${tone === 'alert' ? 'text-[#be123c]' : 'text-[#475569]'}`}>{value}</p>
    </div>
  );
}
...
function BillingStat({ label, value, alert = false }: { label: string; value: string; alert?: boolean }) {
  return (
    <div className={`rounded-[20px] border p-4 ${alert ? 'border-[#fecaca] bg-[linear-gradient(135deg,#fff1f2,#ffffff)]' : 'border-[#e2e8f0] bg-[linear-gradient(135deg,#fbfcff,#ffffff)]'}`}>
      <p className="mb-1 text-[10px] font-bold uppercase tracking-[0.18em] text-[#94a3b8]">{label}</p>
      <p className={`text-lg font-semibold ${alert ? 'text-[#be123c]' : 'text-[#0f172a]'}`}>
        {value}
      </p>
    </div>
  );
...
function EmptyBlock({ icon, label, compact = false }: { icon: string; label: string; compact?: boolean }) {
  return (
    <div className={`text-center ${compact ? 'py-8' : 'py-16'}`}>
      <div className="mb-3 text-[#cbd5e1]">{materialIcon(icon)}</div>
...
function SidebarInfoRow({ icon, label, value }: { icon: string; label: string; value: string }) {
  return (
    <div className="flex items-start gap-4">
      <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-[18px] border border-[#e2e8f0] bg-white text-[#94a3b8] shadow-[0_6px_20px_rgba(15,23,42,0.04)]">
        {materialIcon(icon)}
      </div>
      <div className="min-w-0 pt-1">
        <p className="text-[10px] font-semibold uppercase tracking-[0.18em] text-[#cbd5e1]">{label}</p>
        <p className="mt-1 text-[1.05rem] font-medium leading-snug text-[#0f172a]">{value}</p>
      </div>
    </div>
  );
}
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between gap-4">
        ...
        <span className="rounded-full bg-white/70 px-3 py-2 text-[#0f172a] shadow-[inset_0_1px_0_rgba(255,255,255,0.8)]">
