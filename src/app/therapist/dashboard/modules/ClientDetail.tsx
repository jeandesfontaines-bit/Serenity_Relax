...
export default function ClientDetail({
  ...
  return (
    <div className="flex-1 overflow-auto bg-[#FDFDFB] text-neutral-900 scrollbar-hide">
      <main className="max-w-[1600px] mx-auto p-16 lg:p-24 space-y-20">
        
        {/* ── Page Header ── */}
        <div className="flex items-end justify-between border-b border-neutral-100 pb-12">
          <div className="flex items-center gap-10">
            <button onClick={onClose} className="w-16 h-16 flex items-center justify-center rounded-full bg-white border border-neutral-100 shadow-sm hover:scale-110 transition-all group">
              <ArrowLeft className="text-neutral-400 group-hover:-translate-x-1 transition-transform" size={24} strokeWidth={2.5} />
            </button>
            <div>
              <p className="text-[10px] font-bold uppercase tracking-[0.5em] text-neutral-400 mb-2 leading-none">DOSSIER PATIENT</p>
          ...
          <div className="flex items-center gap-6">
              <button 
                ...
                className="h-16 px-10 rounded-full bg-neutral-900 text-white text-[10px] font-bold uppercase tracking-[0.4em] hover:scale-105 active:scale-95 transition-all shadow-2xl flex items-center gap-4"
              >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[450px_1fr] gap-24 items-start">
          ...
          <aside className="space-y-12 sticky top-24">
            <div className="bg-white rounded-[4rem] p-12 border border-neutral-100 shadow-[0_40px_80px_-20px_rgba(0,0,0,0.05)] space-y-12">
              <div className="flex flex-col items-center text-center space-y-10">
                <div className="relative group">
                  ...
                  <div className="absolute -bottom-4 -right-4 w-16 h-16 bg-white rounded-full flex items-center justify-center shadow-xl border border-neutral-50 text-neutral-300">
                ...
                <div className="space-y-3">
                  ...
                  <div className="text-[12px] font-bold uppercase tracking-[0.3em] text-neutral-400">
                    <EditableField 
                      label="Email" 
              ...
              <div className="space-y-10 pt-12 border-t border-neutral-50">
                <SidebarRow 
              ...
              <div className="pt-10">
                <button className="w-full flex items-center justify-center gap-4 h-16 rounded-full border-2 border-neutral-50 text-[10px] font-bold uppercase tracking-[0.4em] text-neutral-400 hover:text-neutral-900 hover:border-neutral-900 transition-all group">
            ...
            <div className="grid grid-cols-2 gap-6">
               <div className="bg-neutral-900 rounded-[3rem] p-10 text-white shadow-2xl space-y-2 group hover:-translate-y-1 transition-transform">
                  ...
                  <p className="text-[9px] font-bold uppercase tracking-[0.1em] opacity-20">SÉANCES</p>
               </div>
               <div className={`rounded-[3rem] p-10 shadow-xl space-y-2 group hover:-translate-y-1 transition-transform ${unpaidCount > 0 ? 'bg-red-500 text-white' : 'bg-white border border-neutral-100'}`}>
                  <p className="text-[10px] font-bold uppercase tracking-[0.4em] opacity-40">OUVERTES</p>
                  ...
                  <p className="text-[9px] font-bold uppercase tracking-[0.1em] opacity-20">FACTURES</p>
               </div>
            </div>
          </aside>
            ...
            <section className="space-y-12">
              <div className="flex items-center justify-between border-b border-neutral-100 pb-8">
                <div>
                  ...
                  <p className="text-[10px] font-bold uppercase tracking-[0.5em] text-neutral-400 mt-4 leading-none">PLANIFICATION ACTIVE</p>
                </div>
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                {confirmedAppts.length > 0 ? (
                  ...
                  <div className="col-span-full py-32 flex flex-col items-center justify-center bg-neutral-50 rounded-[4rem] border-2 border-dashed border-neutral-100 group">
                    <Calendar size={64} strokeWidth={1} className="text-neutral-200 mb-8 group-hover:scale-110 transition-transform duration-700" />
                    <p className="text-neutral-300 font-bold text-[12px] uppercase tracking-[0.4em]">Aucune planification en cours</p>
                  </div>
                )}
              </div>
            </section>
            ...
            <section className="space-y-12">
              <div className="flex items-center justify-between border-b border-neutral-100 pb-8">
                <div>
                  ...
                  <p className="text-[10px] font-bold uppercase tracking-[0.5em] text-neutral-400 mt-4 leading-none">CHRONOLOGIE DES SOINS</p>
                </div>
              </div>
              <div className="space-y-6">
                {completedAppts.length > 0 ? (
                  ...
                  <div className="py-20 px-12 bg-neutral-50 rounded-[3rem] border border-neutral-100">
                    <p className="text-neutral-400 font-bold text-xl tracking-tighter">Nouveau patient sans historique enregistré.</p>
                  </div>
                )}
              </div>
            </section>
  ...
  return (
    <div className="flex items-center gap-8 group">
      ...
      <div className="flex-1">
        <p className="text-[10px] font-bold uppercase tracking-[0.4em] text-neutral-400 mb-2 leading-none">{label}</p>
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between mb-20">
        <div className={`flex items-center gap-4 text-[12px] font-bold uppercase tracking-[0.3em] ${isDark ? 'text-white/40' : 'text-neutral-300'}`}>
          <Clock size={18} strokeWidth={2.5} />
          <span>{appt.time || '10:00'} — {appt.endTime || '11:00'}</span>
        </div>
        <div className={`w-14 h-14 flex items-center justify-center rounded-full transition-all ${isDark ? 'bg-white/10 text-white' : 'bg-neutral-50 text-neutral-300'} group-hover:scale-110`}>
      ...
      <div className="flex items-center gap-6">
        ...
        <span className={`${isDark ? 'text-white/20' : 'text-neutral-200'} font-bold text-[12px] tracking-widest`}>
...
function HistoryRow({ appt, onClick }: { appt: Appointment, onClick: () => void }) {
  ...
  return (
    <button 
      onClick={onClick}
      className="w-full bg-white border border-neutral-50 rounded-[3rem] p-10 flex items-center gap-12 hover:bg-neutral-900 hover:text-white transition-all group shadow-sm hover:shadow-[0_40px_80px_-20px_rgba(0,0,0,0.15)] hover:-translate-y-1 relative overflow-hidden"
    >
      <div className="flex flex-col items-center justify-center w-24 h-24 border-r border-neutral-100 pr-12 group-hover:border-white/10 transition-colors">
        <span className="text-4xl font-bold leading-none tracking-tighter">{day}</span>
        <span className="text-[11px] font-bold text-neutral-400 tracking-[0.4em] mt-3 group-hover:text-white/40 leading-none">{month}</span>
      </div>
      <div className="flex-1 text-left">
        ...
        <div className="flex items-center gap-6 mt-4">
          <div className="flex items-center gap-3">
             <Clock size={14} strokeWidth={2.5} className="text-neutral-300 group-hover:text-white/20" />
             <p className="text-[11px] font-bold text-neutral-400 uppercase tracking-[0.2em] group-hover:text-white/40">
      ...
      <div className="flex items-center gap-10">
        <div className={`px-8 py-3 rounded-full text-[10px] font-bold uppercase tracking-[0.2em] shadow-sm transition-all border ${
          appt.paid 
            ...
            : 'bg-neutral-50 border-neutral-100 text-neutral-400 group-hover:bg-white/10 group-hover:border-white/10 group-hover:text-white/40'
        }`}>
          {appt.paid ? 'PAYÉ' : 'EN ATTENTE'}
        </div>
        <div className="w-14 h-14 flex items-center justify-center rounded-full text-neutral-200 group-hover:text-white group-hover:scale-110 transition-all">
...
function EditableField({
  ...
  return editing ? (
    ...
    <div
      ...
      className="cursor-text hover:bg-neutral-50 rounded-xl px-2 -mx-2 transition-all flex items-center group/edit"
    >
      <span className="truncate">{value || <span className="text-neutral-200 font-normal">{label}</span>}</span>
      <Edit3 size={14} className="ml-3 text-neutral-200 opacity-0 group-hover/edit:opacity-100 transition-opacity" />
    </div>
  );
}
