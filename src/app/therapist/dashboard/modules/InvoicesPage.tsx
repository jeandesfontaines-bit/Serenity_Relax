      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-10 bg-white/40 backdrop-blur-3xl p-12 rounded-[3.5rem] border border-white shadow-2xl shadow-indigo-100/10 relative overflow-hidden">
        ...
        <div className="w-full md:w-auto flex flex-col sm:flex-row gap-4 relative z-10">
           <div className="relative group flex-1 md:min-w-[320px]">
              <Search className="absolute left-6 top-1/2 -translate-y-1/2 text-gray-300 group-hover:text-[#5F27CD] transition-colors" size={18} />
              <input 
                type="text" 
                placeholder="Rechercher une facture..." 
                value={search}
                onChange={e => setSearch(e.target.value)}
                className="w-full h-16 bg-white/60 border border-white rounded-full pl-16 pr-8 text-lg font-serif italic focus:outline-none focus:ring-8 focus:ring-indigo-50 transition-all shadow-sm"
              />
           </div>
        </div>
      </header>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50/50">
                ...
                <th className="px-10 py-6 text-[0.65rem] font-black uppercase tracking-widest text-gray-400 text-right text-right">Actions</th>
              </tr>
            </thead>
