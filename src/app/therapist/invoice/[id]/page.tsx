    ...
    <>
      ...
      <div className="min-h-screen bg-[#F8F5F0] pt-32 pb-24 px-6 md:px-12">
        <div className="max-w-4xl mx-auto space-y-10">
          ...
          <button 
            onClick={() => router.back()} 
            className="flex items-center gap-3 text-gray-400 font-black text-[0.65rem] uppercase tracking-widest hover:text-[#059669] transition-colors"
          >
            <ArrowLeft className="w-5 h-5" /> Retour à l&apos;archive
          </button>

          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} className="dash-card p-6 bg-white border border-gray-100 rounded-xl shadow-sm lg:p-16 border border-white relative overflow-hidden">
            
            {/* ── HEADER SENSORIEL ── */}
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-5 border-b border-gray-100 pb-12 mb-12">
              <div className="space-y-4">
                <div className="flex items-center gap-3">
                   <div className="w-8 h-8 rounded-xl bg-[#059669] text-white flex items-center justify-center animate-pulse"><FileText size={16} /></div>
                   <p className="text-[0.6rem] font-black uppercase tracking-[0.4em] text-[#059669]">Facturation Détaillée</p>
                </div>
                <h1 className="title-luxe text-2xl md:text-3xl">{invoice.id}</h1>
              </div>
              <div className="text-right">
                <p className="text-lg font-sans italic text-gray-400">{invoice.date}</p>
                <p className="text-3xl font-light text-[#222F3E] mt-2">{invoice.amount} <small className="text-xl font-black opacity-20">CHF</small></p>
              </div>
            </div>
              ...
              <div className="space-y-8">
                <div>
                  ...
                  <div className="flex items-center gap-4">
                     <div className="w-12 h-12 bg-[#F8F5F0] rounded-2xl flex items-center justify-center text-[#059669]"><User size={24} /></div>
                ...
                <div>
                  ...
                  <div className="flex items-center gap-4">
                     <div className="w-12 h-12 bg-[#F8F5F0] rounded-2xl flex items-center justify-center text-[#10B981]"><Sparkles size={24} /></div>
                ...
                <div className="relative pl-10 space-y-12">
                   ...
                   {invoice.status === 'paid' && (
                     <motion.div initial={{ x: -20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} className="relative">
                        <div className="absolute -left-10 top-0 w-6 h-6 bg-[#34D399] border-4 border-white rounded-full flex items-center justify-center shadow-lg shadow-emerald-100" />
                        <div>
                           <p className="text-sm font-bold text-[#34D399]">Règlement Confirmé</p>
            ...
            {invoice.status === 'pending' && (
              <div className="mt-20">
                <button
                  ...
                  className="w-full h-24 bg-slate-50 text-slate-700 rounded-xl border border-slate-100 flex items-center justify-between px-6 group hover:bg-slate-100 transition-all font-sans italic text-xl"
                >
                    ...
                    >
                      {[
                        ...
                        <motion.button
                          ...
                          className="flex flex-col items-center justify-center gap-5 py-10 glass rounded-[1.5rem] border border-white hover:border-[#059669]/20 transition-all shadow-sm"
                        >
            ...
            <div className="mt-20 border-t border-gray-50 pt-16">
              <div className="flex items-center justify-between mb-8">
                ...
                <button
                  ...
                  className="flex items-center gap-3 text-[#059669] text-[0.6rem] font-black uppercase tracking-widest hover:underline"
                >
              ...
              {isEditingNotes ? (
                <textarea
                  ...
                  rows={6}
                  className="w-full bg-white/60 backdrop-blur-md border border-[#059669]/20 p-8 rounded-[1.5rem] text-xl font-sans italic focus:outline-none focus:ring-8 focus:ring-emerald-50 transition-all resize-none shadow-inner"
                />
              ) : (
                <div className="bg-[#F8F5F0]/60 p-5 rounded-[1.5rem] text-gray-600 font-sans italic text-xl leading-relaxed italic border border-white">
                  {invoice.notes}
                </div>
              )}
            </div>
            ...
            <div className="mt-20 flex flex-col sm:flex-row gap-6">
              ...
              <button 
                onClick={resendEmail} 
                className="flex-1 flex items-center justify-center gap-4 py-8 text-[0.7rem] font-black uppercase tracking-[0.2em] border border-gray-200 rounded-xl hover:bg-white hover:shadow-xl transition-all"
              >
