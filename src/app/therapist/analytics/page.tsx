  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6 pb-12">
      ...
      <motion.header 
        ...
        className="flex flex-col md:flex-row justify-between items-start md:items-end gap-8 bg-[#222F3E] text-white p-8 lg:p-5 rounded-[1.5rem] shadow-2xl relative overflow-hidden"
      >
        <div className="absolute top-0 right-0 p-16 opacity-5 pointer-events-none -rotate-12"><Activity size={300} /></div>
        
        <div className="space-y-6 relative z-10">
           <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-2xl bg-[#059669] text-white flex items-center justify-center animate-pulse"><BarChart3 size={20} /></div>
              <p className="text-[0.6rem] font-black uppercase tracking-[0.4em] text-[#059669]">Pulse Intelligence</p>
           </div>
        ...
        <div className="dash-card bg-[#222F3E] text-white p-6 flex flex-col justify-between h-40 w-full lg:w-64 shadow-xl shadow-emerald-200/20">
           <p className="text-[0.55rem] font-black uppercase tracking-widest text-[#10B981]">Croissance</p>
           ...
           <div className="h-1 w-full bg-white/10 rounded-full overflow-hidden">
              <motion.div initial={{ width: 0 }} animate={{ width: `${occupancyRate}%` }} className="h-full bg-gradient-to-r from-[#059669] to-[#10B981]" />
           </div>
        </div>
      </motion.header>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-8">
        {[
          ...
          { label: 'Occupation', val: `${occupancyRate}%`, icon: Activity, color: 'text-[#059669]', bg: 'bg-emerald-50' },
          { label: 'Clients actifs', val: activeClients, icon: Users, color: 'text-[#10B981]', bg: 'bg-sky-50' },
          { label: 'Séances', val: sessionsThisMonth, icon: Calendar, color: 'text-[#0F766E]', bg: 'bg-slate-50' }
        ].map((stat, i) => (
          <motion.div 
            key={stat.label}
            ...
            className="dash-card p-6 bg-white border border-gray-100 rounded-xl shadow-sm border border-white hover:shadow-xl transition-all"
          >
      ...
      {/* === ENHANCED DUAL GRAPH === */}
      <div className="dash-card p-6 bg-white border border-gray-100 rounded-xl shadow-sm border border-white space-y-12">
        ...
        <div className="h-96 flex items-end gap-6 md:gap-5">
          {labels.map((day: string, i: number) => (
            <div key={day} className="flex-1 flex flex-col justify-end gap-3 group">
              ...
              <motion.div
                ...
                className="bg-gradient-to-t from-[#059669] to-[#10B981] rounded-[1.5rem] relative flex-1 group-hover:brightness-110 shadow-xl shadow-emerald-100/10 transition-all flex items-end justify-center pb-4"
              >
              ...
              <motion.div
                ...
                className="bg-gradient-to-t from-[#34D399] to-[#10B981] rounded-[1rem] h-8 opacity-60 shadow-lg shadow-emerald-100/10"
              />
              
              <p className="text-center text-[0.65rem] font-black uppercase tracking-widest text-gray-400 mt-4 group-hover:text-[#059669] transition-colors">{day}</p>
            </div>
          ))}
        </div>
        ...
        <div className="flex justify-center gap-12 pt-8 border-t border-gray-50 text-[0.65rem] font-black uppercase tracking-widest">
          <div className="flex items-center gap-3 text-[#059669]">
            <div className="w-5 h-5 bg-gradient-to-t from-[#059669] to-[#10B981] rounded-lg shadow-sm"></div>
            <span>Occupation (%)</span>
          </div>
          <div className="flex items-center gap-3 text-[#34D399]">
            <div className="w-5 h-5 bg-gradient-to-t from-[#34D399] to-[#10B981] rounded-lg shadow-sm"></div>
